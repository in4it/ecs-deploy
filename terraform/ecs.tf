...
resource "aws_ecs_cluster" "cluster" {
  ...
  capacity_providers = var.ecs_capacity_provider_enabled ? [aws_ecs_capacity_provider.deploy[*].name] : []
...
resource "aws_autoscaling_group" "cluster" {
  ...
  name                  = "ecs-${var.cluster_name}-autoscaling-${each.key}"
...
resource "aws_autoscaling_lifecycle_hook" "cluster" {
  count                  = var.ecs_capacity_provider_enabled ? 0 : length(aws_autoscaling_group.cluster)
  name                   = "${var.cluster_name}-hook"
  autoscaling_group_name = aws_autoscaling_group.cluster[count.index].name
...
resource "aws_ecs_capacity_provider" "deploy" {
  count = var.ecs_capacity_provider_enabled ? length(aws_autoscaling_group.cluster) : 0
  name  = "deploy"

  auto_scaling_group_provider {
    auto_scaling_group_arn         = aws_autoscaling_group.cluster[count.index].arn
